...
def moyrl (sel, cours, p):
  tab = tabcours (coursval(cours,sel[0]), p)
  sap (tab)
  # print len(tab), tab[0]
  a = rl(tab)
  # print a
...
def testrlm (sel, cours, p):
  global flagrlm
  global coefsrlm
  flagrlm = 0
  coefsrlm = moyrl (sel, cours, p)
  flagrlm = 1
  r = moyind (sel, repetegsc, [init_indics, etape_indics, graphe_indics, p])
  return r
 
def testcorlm (sel, cours, p):
  global flagrlm
  global coefsrlm
  flagrlm = 0
  coefsrlm = corl (sel, cours, p)
  flagrlm = 1
  r = moyind (sel, repetegsc, [init_indics, etape_indics, graphe_indics, p])
  return r
